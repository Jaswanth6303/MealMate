...
def add_opening_hours(request):
    ...
    if request.user.is_authenticated:
        if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'POST':
            ...
            is_closed = request.POST.get('is_closed') == 'on'  # Convert to boolean
            ...
            try:
                ...
                # Prepare the response based on whether the store is closed
                if hour:
                    day_obj = OpeningHour.objects.get(id=hour.id)
            ...
            # Return 400 status for invalid request
            return HttpResponse('Invalid request', status=400)

    else:
        # Return 401 status if the user is not authenticated
